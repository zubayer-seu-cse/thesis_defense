...
A common misconception is that TLS alone can secure the SDN control plane. In practice, TLS falls short in two fundamental ways. First, TLS provides transport-layer confidentiality and authentication but does not validate the \emph{semantic} integrity of OpenFlow messages. A compromised controller---a realistic threat given SDN's single-point-of-control architecture---can issue well-formed but malicious \texttt{Flow\_Mod} messages that TLS will faithfully encrypt and deliver; this is a controller-level insider threat that TLS was never designed to prevent. Second, TLS secures only the controller--switch channel and has no visibility into the switch's local data plane. A switch compromised through a malicious end-host exploit, physical access, or firmware attack can have its flow tables modified locally, bypassing both the controller and TLS entirely. HMAC-based flow rule verification addresses both gaps: it enables an auxiliary agent to independently verify each flow rule's authenticity regardless of controller trustworthiness, while periodic shadow-table audits detect unauthorised local modifications that occur outside the TLS-protected channel.
...
Implementation uses TensorFlow 2.19.0 on Google Colab (NVIDIA T4 GPU). The Adam optimizer ($\eta = 10^{-3}$), binary cross-entropy loss with class weights, batch size 2,048, early stopping on validation AUC (patience 15), and ReduceLROnPlateau scheduling are employed. Training converges in roughly 30 epochs.
