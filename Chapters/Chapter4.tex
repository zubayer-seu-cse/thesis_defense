...
After PCA transformation and splitting, each data sample is a 24-dimensional vector corresponding to the 24 principal components. To serve as input to the TCN model, which expects three-dimensional input tensors of shape \texttt{(batch\_size, feature\_dimension, channels)}, each sample is reshaped from $(24,)$ to $(24, 1)$. This reshaping formats the 24 PCA components as a multi-dimensional feature representation with a single channel, enabling the 1D convolutional architecture to extract patterns across the feature space.

This representation is effective because: (1) the PCA components are ordered by decreasing variance, establishing a natural ordering where early components capture the most significant patterns; (2) the dilated convolutions of the TCN can learn both local feature interactions (between adjacent components) and global relationships (across the full feature space); and (3) the residual blocks with exponentially increasing dilation factors enable the model to capture multi-scale patterns in the variance hierarchy.
