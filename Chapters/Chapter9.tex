...
Wang et al. \cite{Wang2025TCN_SDN} & 2025 & TCN & InSDN & 97.00 & 96.40 & 96.91 & 96.72 \\
...
\textbf{4. Dataset-Specific Considerations:} Direct comparison across different datasets must be interpreted with caution. Models evaluated on NSL-KDD, CIC-IDS-2017, or UNSW-NB15 cannot be directly compared to models evaluated on InSDN, as the difficulty of classification varies across datasets. The most meaningful comparisons are with models evaluated on InSDN: Said et al.\ \cite{Said2023CNNBiLSTM} (99.90\%), Ataa et al.\ \cite{Ataa2024SDNDL} (99.70\%), Kanimozhi et al.\ \cite{Kanimozhi2025DRL} (98.85\%), Ahmad et al.\ \cite{Ahmad2021CNN_SDN} (99.20\%), El-Sayed et al.\ \cite{elsayed2020insdn} (98.50\%), and Wang et al.\ \cite{Wang2025TCN_SDN} (97.00\%). Notably, Wang et al.\ is the only other TCN-based model evaluated on InSDN; its 2.85-point accuracy deficit relative to TCN-HMAC underscores the importance of the preprocessing pipeline (PCA, Pearson correlation thresholding, class weighting) and flow-level feature engineering employed in this thesis compared with their packet-length-only input design.

\textbf{5. TCN Architecture Variants:} Among TCN variants, the proposed model achieves the highest accuracy. Li et al.'s TCN-SE \cite{LiLi2025TCNSE} (99.62\%), Benfarhat et al.'s TCN+Attention \cite{Benfarhat2025TCN} (99.73\%), Mei et al.'s BiTCN \cite{Mei2024BiTCN} (99.60\%), Deng et al.'s BiTCN-MHSA \cite{Deng2024BiTCNMHSA} (99.72\%), and Wang et al.'s plain TCN \cite{Wang2025TCN_SDN} (97.00\%) all achieve lower accuracy than the proposed model, despite some employing more complex architectural enhancements (squeeze-and-excitation, attention mechanisms, bidirectional processing). Wang et al.'s result on the same InSDN dataset is particularly instructive: their packet-length-only feature design coupled with just three residual blocks yields 97.00\% accuracy and 2.1~ms inference, whereas the proposed model's flow-level feature set with PCA and six dilated residual blocks achieves 99.85\% accuracy at 0.17~ms, a 2.85-point accuracy improvement and 12$\times$ faster inference. This suggests that the simplicity of the proposed TCN architecture, combined with effective preprocessing (PCA, class weighting), is sufficient for high performance without architectural complexity overhead.
